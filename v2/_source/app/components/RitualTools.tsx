...
export function IncenseTool() {
  ...
  return (
    <div className="tool-stage">
      ...
      <button className="button button-gold" onClick={light} disabled={waiting}><Icon name="incense" />{waiting ? "กำลังรอควันธูป…" : number ? "จุดธูปอีกครั้ง" : "จุดธูป"}</button>
    </div>
  );
}
...
export function WalletOpeningTool() {
  ...
  return (
    <div className="tool-stage">
      ...
      <div className="moon-display"><span><Icon name={result.waxing ? "sunrise" : "moon"} /></span><div><small>{result.date.toLocaleDateString("th-TH", { dateStyle: "full" })}</small><h2>{result.phase}</h2><p>อายุจันทร์ประมาณ {Math.round(result.age)} วัน</p></div></div>
      <article className={`auspicious-card ${result.waxing ? "good" : "rest"}`}>
        <span><Icon name={result.waxing ? "sparkle" : "moon"} /></span>
  ...
  return (
    <div className="horse-layout">
      ...
      <article className="chant-card" style={{ "--chant-color": selected.themeColor } as React.CSSProperties}>
        ...
        <div className="chant-counter"><div><small>จำนวนรอบ</small><strong>{round} / {selected.rounds}</strong></div><button className="button button-gold" onClick={() => setRound((current) => current >= selected.rounds ? 0 : current + 1)}><Icon name={round >= selected.rounds ? "sparkle" : "plus"} />{round >= selected.rounds ? "เริ่มนับใหม่" : "นับรอบสวด"}</button></div>
...
export function MoneyChantTool() {
  return (
    <div className="tool-stage">
      <div className="money-symbol"><Icon name="money" /></div>
...
export function WalletGuide() {
  const steps: Array<{ icon: IconName; title: string; body: React.ReactNode; note: React.ReactNode }> = [
    { icon: "wallet-guide", title: "เริ่มต้นด้วยพลัง", body: <>เมื่อได้รับกระเป๋าแล้ว ให้ใส่ <strong>ธนบัตรที่มีมูลค่าสูงที่สุดเท่าที่คุณมี</strong> (ไม่จำกัดสกุลเงิน) ลงไปในกระเป๋า เพื่อเป็นการวางรากฐานความมั่งคั่ง และสร้างแรงดึงดูดเงินก้อนใหญ่หรือทรัพย์สินที่มีมูลค่าสูงเข้ามาหาคุณ</>, note: <>อย่าลืมใส่ <strong>“ซองแดง”</strong> ที่ติดมากับกระเป๋าลงไปด้วย เพื่อความเป็นสิริมงคลสูงสุด</> },
    { icon: "moon", title: "เชื่อมต่อจิตวิญญาณ", body: <>นำกระเป๋าไปไว้ <strong>ใต้หมอน</strong> เพื่อให้กระเป๋าจดจำเจ้าของ และจดจำกลิ่นอายของเงิน</>, note: <>วางไว้กี่คืนก็ได้ตามความสบายใจ · วันแรกที่เริ่มใช้ควรเป็นวันที่อารมณ์ดี · ฤกษ์ดีคือวันข้างขึ้น 1–15 ค่ำ</> },
    { icon: "sparkle", title: "ปลุกเสกด้วยตนเอง", body: <>ท่องคาถาเรียกเงินเพื่อให้พลังงานไหลเวียนและเปิดทางให้โชคลาภ</>, note: <>เปิดหน้าคาถาเรียกเงินเพื่อฟังเสียงและทบทวนบทเต็มได้ทุกเมื่อ</> },
  ];
  return <div className="guide-grid">{steps.map((step, index) => <article key={step.title}><span><Icon name={step.icon} /></span><small>STEP {index + 1}</small><h2>{step.title}</h2><p>{step.body}</p><div>{step.note}</div></article>)}</div>;
}

create sequence if not exists public.order_number_sequence start 1001;
...
as $$
  ...
  from public.orders o where o.public_token_hash = p_public_token_hash;
$$;

create or replace function public.release_order_stock_v1(p_order_id uuid)
returns void
language plpgsql
security definer
set search_path = public
...
begin
  update public.product_skus s
    set reserved_quantity = greatest(0, s.reserved_quantity - i.quantity)
...
begin
  ...
  if now() > v_order.expires_at or p_transaction_at > v_order.expires_at then
    update public.orders set status = 'needs_review' where id = p_order_id;
    return 'needs_review';
  end if;
...
begin
  with expired as (
    ...
    where status in ('pending_payment', 'verification_failed') and expires_at < now()
    returning id
  ), released as (
    update public.product_skus s
      set reserved_quantity = greatest(0, s.reserved_quantity - i.quantity)
    from public.order_items i, public.products p, expired e
    where i.order_id = e.id and i.sku_id = s.id
      and p.id = i.product_id and p.track_inventory = true
    returning s.id
...
create or replace function public.activate_payment_account_v1(
  p_account_id uuid,
  p_validation_trans_ref text
) returns jsonb
language plpgsql
security definer
set search_path = public
...
begin
  perform pg_advisory_xact_lock(hashtext('meemon-payment-account-switch'));
  if exists (select 1 from public.orders where status in ('pending_payment', 'verifying')) then
    raise exception 'OPEN_ORDERS';
  end if;
  select * into v_account from public.payment_accounts
    where id = p_account_id and status = 'pending_validation' for update;
  if not found then raise exception 'ACCOUNT_NOT_PENDING'; end if;
  update public.payment_accounts set status = 'retired' where status = 'active';
  update public.payment_accounts
    set status = 'active', validated_at = now(), validation_trans_ref = p_validation_trans_ref
    where id = p_account_id returning * into v_account;
  return to_jsonb(v_account);
end;
$$;

create or replace function public.approve_review_payment_v1(p_order_id uuid)
returns void
language plpgsql
security definer
set search_path = public
...
begin
  select * into v_order from public.orders where id = p_order_id for update;
  if not found or v_order.status <> 'needs_review' then raise exception 'ORDER_NOT_REVIEWABLE'; end if;
...
revoke all on function public.release_order_stock_v1(uuid) from public, anon, authenticated;
...
revoke all on function public.approve_review_payment_v1(uuid) from public, anon, authenticated;
revoke all on function public.activate_payment_account_v1(uuid, text) from public, anon, authenticated;
...
grant execute on function public.release_order_stock_v1(uuid) to service_role;
...
grant execute on function public.approve_review_payment_v1(uuid) to service_role;
grant execute on function public.activate_payment_account_v1(uuid, text) to service_role;
